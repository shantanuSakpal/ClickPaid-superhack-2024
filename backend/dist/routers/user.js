...
const INSTAMOJO_CLIENT_ID = process.env.INSTAMOJO_CLIENT_ID.toString();
const INSTAMOJO_CLIENT_SECRET = process.env.INSTAMOJO_CLIENT_SECRET.toString();
...
const W3_PRIVATE_Key = process.env.W3_PRIVATE_Key.toString();
const W3_PROOF = process.env.W3_PROOF.toString();
...
router.get("/getAccessToken", middleware_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const options = {
        method: "POST",
        headers: {
    ...
    try {
        const response = yield fetch("https://test.instamojo.com/oauth2/token/", options);
        if (!response.ok) {
...
router.post("/getPaymentDetails", middleware_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const response = yield fetch(`https://test.instamojo.com/v2/payments/${payment_id}/`, options);
        const data = yield response.json();
        console.log(data);
        res.status(200).json(data);
    }
